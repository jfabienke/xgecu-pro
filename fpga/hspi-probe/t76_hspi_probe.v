...
`include "probe_obs.vh"
// Most recent word the MCU sent, for RESPONSE_MODE 3 (echo). Declared here
// because the capture assigns it well before the transmitter reads it.
reg [15:0] last_rx = 16'd0;

`include "probe_hd_bus.vh"
...
always @(posedge HTCLK) begin
    ...
    if (HTVLD) begin
        ...
        if (skipped < SKIP[7:0])
            ...
            // Echo mode needs the MCU's most recent word. Taken here rather
            // than sampled separately so it is exactly what the capture
            // recorded -- if the two ever disagreed, the echo would be
            // answering something nobody observed.
            last_rx <= hd_bus[15:0];
            capcnt <= capcnt + 8'd1;
        end
...
// --- response selection -----------------------------------------------------
// One constant per experiment. Rebuild, run, read the tail registers out of the
    ...
    3'd2: begin                                  // clocking the packet out
        if (HTVLD) begin                         // MCU took the bus: abandon
            ...
            if (tx_idx == 5'd0)
                ...
                // RESPONSE_MODE selects what we answer with. The point of this
                // design is to vary it and watch whether the MCU's tail
                // registers change: a failing erase closed over
                // 0x37 3A 32 33 04 and a succeeding one over 0x01 24 27, so the
                // tail is a success/failure signal we can hill-climb on.
                case (RESPONSE_MODE)
                    3'd1:    tx_word <= 16'h0000;      // all zeros
                    3'd2:    tx_word <= 16'hFFFF;      // all ones
                    3'd3:    tx_word <= last_rx;       // echo the MCU's last word
                    default: tx_word <= tx_ctr;        // free-running counter
                endcase
                tx_ctr  <= tx_ctr + 16'd1;
            end else
