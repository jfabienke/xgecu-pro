#!/usr/bin/env python3
"""
Generate ports and pin constraints for the VCC probe.
...
def main():
    ...
    (here / "vccprobe_ports.vh").write_text("\n".join(ports) + "\n")
    ...
    a = ["# T76 VCC probe -- generated, do not edit.",
    ...
    for net, v in list(RAIL.items()) + list(ISP_POWER.items()):
        ...
        if ball:
            ...
            missing.append(net)
    (here / "t76_vccprobe.adc").write_text("\n".join(a) + "\n")
    (here / "t76_vccprobe.sdc").write_text(
    ...
    (here / "build.tcl").write_text(
        ...
        "optimize_rtl\noptimize_gate\nlegalize_phy_inst\nplace\nroute\n"
        "bitgen -bit t76_vccprobe.bit -version 0X00 "
        "-g ucode:000000000000000000000000 -info -log_file t76_vccprobe_bit.log\n"
        "exit\n")
    ...
    print("  wrote vccprobe_ports.vh, t76_vccprobe.adc/.sdc, build.tcl")
