#!/usr/bin/env python3
"""
Generate ports and pin constraints for the rail sweep.
...
def main():
    ...
    (here / "railsweep_ports.vh").write_text("\n".join(ports) + "\n")
    ...
    a = ["# T76 rail sweep -- generated, do not edit.",
    ...
    for net, v in list(RAIL.items()) + list(ISP_POWER.items()):
        ...
        if ball:
            ...
            missing.append(net)
    (here / "t76_railsweep.adc").write_text("\n".join(a) + "\n")
    (here / "t76_railsweep.sdc").write_text(
    ...
    (here / "build.tcl").write_text(
        ...
        "optimize_rtl\noptimize_gate\nlegalize_phy_inst\nplace\nroute\n"
        "bitgen -bit t76_railsweep.bit -version 0X00 "
        "-g ucode:000000000000000000000000 -info -log_file t76_railsweep_bit.log\n"
        "exit\n")
    ...
    print("  wrote railsweep_ports.vh, t76_railsweep.adc/.sdc, build.tcl")
