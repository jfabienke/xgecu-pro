...
- **`algorithm.xml`** (48 MB, generated by `dump-alg-minipro.bash`) — 649 T76
  FPGA bitstreams (base64, zlib-compressed), **firmware-version specific** (pinned
  to XGPro V13.19 / device fw 00.1.17).
- **`infoic.xml`** — chip database (34,607 T76 devices; the one commit here added
  2,028 V13.19 chips). Extracted from `InfoICT76.dll` by the `dumpic/` tools.
- **`logicic.xml`** — logic-IC test vectors. `firmware-*.dat` — bundled firmware
  images for `--update`.
