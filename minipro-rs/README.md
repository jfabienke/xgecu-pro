...
| `minipro-db` | `ChipDb` trait with four backends: `XmlDb` (infoic.xml), `DllDb` (parses `InfoICT76.dll` directly), `HttpDb` (`net` feature — provisions the DLL from a mirror into RAM, persists only the derived catalog + `.alg` bitstreams, daily version check), `RarDb` (`rar` feature, **on by default** — reads a vendor `.rar`/SFX `.exe` in place, decompressing to memory, persisting nothing; links unrar), and `vendor` (the zero-setup default: fetches XGecu's installer archive once, caches it, and reads it via `RarDb`). |
