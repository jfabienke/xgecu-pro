...
//!    the length, then the raw bitstream in one transfer, vs
...
const CMD_WRITE_BITSTREAM: u8 = 0x26; // single-shot bitstream upload
const CMD_WRITE_BITSTREAM2: u8 = 0x2a; // multipart (logic TTL1/TTL2)
...
impl T56 {
    ...
    fn upload_named(&mut self, name: &str, bits: &[u8]) -> Result<()> {
        ...
        le32(&mut hdr, 4, bits.len().min(u32::MAX as usize) as u32);
        self.send(&hdr)?;
        self.tx.send(EP_MSG_OUT, bits)?; // raw bitstream, one shot
...
impl Programmer for T56 {
    ...
    fn identify(&mut self, s: &Session) -> Result<ChipId> {
        ...
        msg[0] = CMD_READID;
        let resp = self.cmd(&msg, 32)?;
        let id_type = wire::at(&resp, 0)?;
        let id_length = s.device.chip_id_bytes.min(4);
        // Total read: the id width comes from the DB and the payload from the
        // device, so bound them against the actual reply rather than assuming.
        let bytes = wire::slice_at(&resp, 2, usize::from(id_length))?;
...
mod fuzz_replies {
    ...
    proptest! {
        #[test]
        fn t56_ops_never_panic(reply in proptest::collection::vec(any::<u8>(), 0..96)) {
            let mut p = T56::new(Box::new(FuzzTx { reply }));
            let s = session();
            let _ = p.query_info();
            let _ = p.identify(&s);
            let _ = p.reset();
